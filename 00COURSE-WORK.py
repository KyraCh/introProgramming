        ...
        while True:
            ...
            if user_input == '1':
                ...
                print("Capacity by camp: \n", self.camps_db[[
                      column_headers[7], column_headers[9]]])
            elif user_input == "6":
...
class Admin(CentralFunctions):
                ...
                if file_extension == '.csv':
                    ...
                    for index in df2.index[::-1]:
                        if (df2['Emergency ID'][index])[:2] == country_code:
                            emergency_id = country_code + \
                                str(int((df2['Emergency ID'][index][2:]))+1)
                            break
                    if emergency_id == '':
    ...
    def close_emergency(current_user, emergency_id, close_date):
        ...
        try:
            ...
            closeDate = datetime.datetime.strptime(
                close_date, "%Y-%m-%d").strftime("%d/%m/%Y")
            # readind data inside csv file and turning them to a panda dataframe.
    ...
    def amend_camps(self):
        ...
        choose_emergency = input(
            "Choose emergency for which you want to edit:")
        choose_emergency = choose_emergency.upper()
            ...
            if user_input == '1':
                while True:
                    try:
                        ...
                        ind = list_of_camps[list_of_camps["Emergency ID"]
                                            == choose_emergency].index.values
                        ...
                        print(
                            "Capacity is smaller than the number of refugees, try again!")

                ind = list_of_camps[list_of_camps["Emergency ID"]
                                    == choose_emergency].index.values
                list_of_camps.loc[ind[0], ['Capacity']] = cap
                print(
                    list_of_camps[list_of_camps["Emergency ID"] == choose_emergency])
    ...
    def write_volunteer(self):
        ...
        try:
            new_vol = pd.DataFrame(
                {'Username': username, 'Camp ID': camp}, index=[0])
        ...
        # add new volunteer to list of users and save updated file
            new_account = pd.DataFrame({'username': [username], 'password': [
                                       password], 'role': ['volunteer'], 'activated': ['TRUE']}, index=[0])
        ...
        while True:
            ...
            if user_input == '1':
                ...
                while True:
                    user_input = input("To confirm change of data, enter [y]:")
                    if user_input == "y":
                        vol_df.at[current_user,
                                  "Second name "] = new_second_name
                        vol_df.to_csv("volunteer_database.csv")
                        print(
                            f"Second name has been set to {new_second_name}.")
                        break
                    else:
                        print("Invalid input.")
            elif user_input == '3':
                ...
                while True:
                    ...
                    if user_input == "y":
                        vol_df.at[current_user,
                                  "Availability "] = f"{new_availability}h"
                        vol_df.to_csv("volunteer_database.csv")
                        print(
                            f"Availability has been set to {new_availability}.")
                        break
                    else:
                        print("Invalid input.")
            elif user_input == "5":
